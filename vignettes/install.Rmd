...
# **Install `massconverter` from github**

You can just use the `devtools` or `remotes` to install `massconverter`. 
...
Then install `massconverter`:

```{r,eval=FALSE,warning=FALSE, R.options="", message=FALSE, cache=TRUE}
devtools::install_github("tidymass/massconverter", dependencies = TRUE)
```
...
```
Error: Failed to install 'massconverter' from GitHub: HTTP error 403. API rate limit exceeded for 171.66.10.237. (But here's the good news: Authenticated requests get a higher rate limit. Check out the documentation for more details.)
```
...
`raster` is a package in [CRAN](https://cran.r-project.org/web/packages/raster/index.html), try to install it first and then install `massconverter`.
...
`Cario` is a package in [CRAN](https://cran.r-project.org/web/packages/Cairo/index.html), try to install it first and then install `massconverter`.
